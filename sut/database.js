...
function initDatabase() {
    db.serialize(() => {
        ...
        for (let i = 1; i <= 300; i++) {
            insertUser.run(`User ${i}`, `user${i}@eshop.com`, 'Password123!', 'user');
        }
        insertUser.finalize();
        ...
        insertProduct.run('Lót chuột Gaming Speed RGB 80x30cm', 100000, 'Chất liệu vải cao cấp, chống trượt, viền LED RGB', 'https://placehold.co/300x300/png?text=Mousepad+RGB', 3);
        insertProduct.run('Chuột không dây Logitech Pebble M350', 300000, 'Thiết kế mỏng nhẹ, click êm ái chống ồn Silent Touch', 'https://placehold.co/300x300/png?text=Logitech+Pebble', 3);
        insertProduct.run('Tai nghe Gaming Havit H2002D', 500000, 'Âm thanh vòm sống động, micro lọc ồn đàm thoại rõ nét', 'https://placehold.co/300x300/png?text=Havit+H2002D', 3);
        ...
        insertCoupon.run('NOACTIVE', 'percent', 20, 100000, '2099-12-31', 0, 2);
